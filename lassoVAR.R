...
lasso <- function(data, q = 1, post = TRUE, intercept = TRUE,
  ...
  rel_diffs_ups <- matrix(NA, k, 1)         # relative changes in penalty loadings
  if (post == TRUE) { # if refitting requested...
  ...
  fit <- list(
    ...
    full_rank_post_init = full_rank_post_init,        # flag full rank post-initial
    full_rank_post_refi = full_rank_post_refi[, 1:l], # flag full rank post-refined path
    k_term = l,                                       # number of loading updates performed
    rel_diff_ups_term = rel_diff_ups,                 # relative change in penalty loadings at termination
    rel_diffs_ups = rel_diffs_ups[1:l, 1]             # relative changes in penalty loadings path
    )
  return(fit)
}
...
ls_sol <- function(x, y) {
  full_rank <- ncol(x) == rankMatrix(x)[1] # check for full column rank
