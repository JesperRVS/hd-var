...
plot_rel_stat_method_by_design <- function(stat, met_plt, des_plt) {
...
# 1. Comparing Lasso and Sqrt-Lasso w/ and w/o post-selection refitting
...
for (thisstat in seq_along(stats_plt)) {
  ...
  plt <- plot_rel_stat_method_by_design(stats_plt[thisstat],
                                    met_plt = lasso_sqrtlasso,
                                    des_plt = des_plt)
  file_name <- paste("all_lasso_sqrtlasso_methods_", "rel_",
...
# plt <- plot_rel_stat_method_by_design("mean", lasso_sqrtlasso, des_plt)
# plt

# where_met_plt <- which_methods(lasso_sqrtlasso)
# where_des_plt <- which_designs(des_plt)
# mean_max_ell2 <- apply(max_ell2_errors, c(2, 3, 4, 5), stat_fctn("mean"))
# mean_max_ell2_plt <- mean_max_ell2[, , where_des_plt, where_met_plt]
# rel_mean_max_ell2_plt <- array(NA, dim(mean_max_ell2_plt))
# dimnames(rel_mean_max_ell2_plt) <- dimnames(mean_max_ell2_plt)
# for (i in seq_along(where_des_plt)) {
#   for (j in seq_along(where_met_plt)) {
#     rel_mean_max_ell2_plt[, , i, j] <- mean_max_ell2_plt[, , i, j] /
#                                        mean_max_ell2_plt[, , i, 1]
#   }
# }
...
# rel_mean_max_ell2 <- sweep(mean_max_ell2, 1, mean_max_ell2[1, , , ], "/")


# create a loop that runs through all where_des_plt and where_met_plt
# and creates an array with the elements of each matrix mean_max_ell2_plot[,,i,j]
# 
